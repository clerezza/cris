...
public class GraphIndexerTest {
    ...
    Graph definitions = new SimpleGraph();
    WatchableGraph dataGraph = new WatchableGraphWrapper(new SimpleGraph());
    GraphIndexer service = null;
    IRI ownsPetProperty = new IRI("http://example.org/pet#owns");

    private void createPerson(String firstName, String lastName) {
        GraphNode node = new GraphNode(new IRI(Util.createURN5()), dataGraph);
    ...
    private void createDefinition(IRI rdfType, List<IRI> properties) {
        GraphNode node = new GraphNode(new BlankNode(), definitions);
    ...
    public void setUp() {
        ...
        GraphNode nodeB = new GraphNode(new IRI(Util.createURN5()), dataGraph);
        nodeB.addProperty(RDF.type, FOAF.Person);
        nodeB.addProperty(FOAF.homepage, new IRI("http://myhomepage/foo?query=bla&bla=test"));
        ...
        createPerson("Harry Joe", "Wotsit-Bloggs");
        {
            ...
            {
                //and another pet
                BlankNodeOrIRI pet = new IRI("http://example.org/pets/Robi");
                node.addProperty(ownsPetProperty, pet);
                GraphNode petNode = new GraphNode(pet, dataGraph);
    ...
    @Test
    public void findResourcesViaIRI() throws ParseException, InterruptedException {
        List<BlankNodeOrIRI> results;
        Thread.sleep(1000);
    ...
    public void findMultiProperties() throws InterruptedException, ParseException {
        ...
        Thread.sleep(1000);
        List<BlankNodeOrIRI> results = service.findResources(conditions);
        Assert.assertEquals(1, results.size());
    }
    ...
    public void lateAddition() {
        try {
            ...
            Thread.sleep(1000);
            List<BlankNodeOrIRI> results = service.findResources(FOAF.firstName, "*Joe*");
    ...
    public void lateAdditionInverse() throws InterruptedException, ParseException {
        ...
        Thread.sleep(1000);
        List<BlankNodeOrIRI> results = service.findResources(FOAF.firstName, "*Jane*");
        Assert.assertEquals(2, results.size());
        node.addProperty(RDF.type, FOAF.Person);
        Thread.sleep(1000);
        List<BlankNodeOrIRI> results2 = service.findResources(FOAF.firstName, "*Jane*");
    ...
    @Test
    @Ignore
    //not working because of CLEREZZA-1014
    ...
    public void reIndexTest() throws InterruptedException {
        ...
        }
        {
            try {
                ...
                List<BlankNodeOrIRI> results = service.findResources(RDFS.comment, "*two*");
                Assert.assertEquals(2, results.size());
            } catch (ParseException ex) {
            }
        }
    ...
    public void joinProperty() {
        try {
            ...
            Thread.sleep(2000);
            {
                ...
                List<BlankNodeOrIRI> results = service.findResources(joinProperty, "*John*");
                Assert.assertEquals(2, results.size());

            }
            {

                List<BlankNodeOrIRI> results = service.findResources(joinProperty, "John Doe");
                Assert.assertEquals(1, results.size());
            ...
            {
                //check before
                List<BlankNodeOrIRI> results = service.findResources(joinProperty, "*Joe*");
                Assert.assertEquals(2, results.size());
            }
            createPerson("Another Joe", "Simpsons");
            Thread.sleep(2000);
    ...
    public void pathVirtualProperty() {
        try {
            List<IRI> pathProperties = new java.util.ArrayList<IRI>();
            ...
            Thread.sleep(1000);
            {
                List<BlankNodeOrIRI> results = service.findResources(pathProperty, "Silvio");
                Assert.assertEquals(1, results.size());
            }
            {
                List<BlankNodeOrIRI> results = service.findResources(pathProperty, "Beni");
                Assert.assertEquals(1, results.size());
            }
            {
                List<BlankNodeOrIRI> results = service.findResources(pathProperty, "Robi");
                Assert.assertEquals(1, results.size());
            }
            {
                List<BlankNodeOrIRI> results = service.findResources(pathProperty, "Robert");
                Assert.assertEquals(1, results.size());
            }
            ...
            GraphNode pet = new GraphNode(new IRI("http://example.org/pet"), dataGraph);
            //GraphNode pet = new GraphNode(new BlankNode(), dataGraph);
            ...
            Thread.sleep(1000);
            {
                List<BlankNodeOrIRI> results = service.findResources(pathProperty, "Fifi");
                Assert.assertEquals(1, results.size());
            }
            ...
            Thread.sleep(1000);
            {
                ...
                List<BlankNodeOrIRI> results = service.findResources(conditions, facetCollector);
    ...
    public void facetCollectorTest() throws InterruptedException, ParseException {
        ...
        Thread.sleep(1000);
        {
            List<BlankNodeOrIRI> results = service.findResources(firstName, "*", false, facetCollector);
            Assert.assertTrue(results.size() > 0);
    ...
    public void sortResultsTest() throws InterruptedException, ParseException {
        ...
        Thread.sleep(1000);
        {
            List<BlankNodeOrIRI> results = service.findResources(firstName, "*", false, sortSpecification);
            Assert.assertTrue(results.size() > 0);
            ...
            for (BlankNodeOrIRI result : results) {
    ...
    public void paginationTest() throws InterruptedException, ParseException {
        ...
        Thread.sleep(1000);
        {
            ...
            List<BlankNodeOrIRI> results = service.findResources(fl, sortSpecification,
